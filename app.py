...
def current_user(request: Request):
    tok  = request.cookies.get("sm_session")
...
:root{--pur:#8b5cf6;--pur-l:#a78bfa;--pur-d:#7c3aed;
  --card:#fff;--surf:#faf5ff;--text:#1e1b4b;--muted:#6b7280;--bdr:#e9d5ff;
  --sh:0 4px 24px rgba(139,92,246,.14);}
...
def auth_page(error="", success="", tab="login"):
    ...
    login_active = "color:#8b5cf6;border-bottom:2.5px solid #8b5cf6;" if tab=="login" else "color:#9ca3af;border-bottom:2.5px solid transparent;"
    reg_active   = "color:#8b5cf6;border-bottom:2.5px solid #8b5cf6;" if tab=="reg"   else "color:#9ca3af;border-bottom:2.5px solid transparent;"
    login_pane   = "block" if tab=="login" else "none"
    reg_pane     = "block" if tab=="reg"   else "none"
    return HEAD + f"""
      ...
      <div style="display:flex;border-bottom:2px solid #e9d5ff;margin-bottom:22px;">
        <button onclick="showTab('login')" id="tab-login"
          style="flex:1;padding:10px;border:none;background:transparent;font-weight:700;
                 font-size:.92rem;cursor:pointer;margin-bottom:-2px;{login_active}">Login</button>
        <button onclick="showTab('reg')" id="tab-reg"
          style="flex:1;padding:10px;border:none;background:transparent;font-weight:700;
                 font-size:.92rem;cursor:pointer;margin-bottom:-2px;{reg_active}">Register</button>
      </div>
      <div id="pane-login" style="display:{login_pane}">
        ...
        </form>{msg if tab=="login" else ""}
      </div>
      <div id="pane-reg" style="display:{reg_pane}">
...
function showTab(t) {{
  ['login','reg'].forEach(function(id) {{
    var active = id===t;
    document.getElementById('pane-'+id).style.display = active?'block':'none';
    var b = document.getElementById('tab-'+id);
    b.style.color = active?'#8b5cf6':'#9ca3af';
    b.style.borderBottom = active?'2.5px solid #8b5cf6':'2.5px solid transparent';
  }});
}}
</script>""" + FOOT

# ── Chat page ──────────────────────────────────────────────────────────────────
def chat_page(username, history, hist_items, active_sid=""):
    hist_rows = ""
    for it in hist_items:
        title  = it.get("title","Untitled")[:34]
        sid    = it["id"]
        active = "background:#f5f3ff;border-color:#e9d5ff;color:#8b5cf6;" if sid==active_sid else ""
        hist_rows += f"""<a href="/chat/{sid}" style="display:block;padding:9px 11px;
          border-radius:11px;margin-bottom:3px;font-size:.83rem;color:#374151;
          border:1px solid transparent;white-space:nowrap;overflow:hidden;
          text-overflow:ellipsis;font-weight:500;text-decoration:none;{active}"
          onmouseover="this.style.background='#f5f3ff';this.style.color='#8b5cf6';this.style.borderColor='#e9d5ff'"
          onmouseout="this.style.background='{"#f5f3ff" if sid==active_sid else ""}';this.style.color='{"#8b5cf6" if sid==active_sid else "#374151"}';this.style.borderColor='{"#e9d5ff" if sid==active_sid else "transparent"}'">
          💬 {title}</a>"""
    if not hist_rows:
    ...
    for msg in history:
        txt = (msg["content"]
               ...
               .replace("\n","<br/>"))
        if msg["role"] == "user":
            bubbles += f"""<div style="display:flex;justify-content:flex-end;margin-bottom:14px;">
              <div style="background:linear-gradient(135deg,#8b5cf6,#a78bfa);color:#fff;
                border-radius:20px 20px 4px 20px;padding:12px 16px;max-width:70%;
                font-size:.92rem;line-height:1.5;box-shadow:0 3px 12px rgba(139,92,246,.28);">
                {txt}</div></div>"""
        else:
            bubbles += f"""<div style="display:flex;justify-content:flex-start;margin-bottom:14px;">
              <div style="background:#fff;color:#1e1b4b;
                border-radius:20px 20px 20px 4px;padding:12px 16px;max-width:70%;
                font-size:.92rem;line-height:1.5;
                box-shadow:0 4px 24px rgba(139,92,246,.14);border:1px solid #e9d5ff;">
                {txt}</div></div>"""
    ...
    <div style="display:flex;flex-direction:column;align-items:center;
      ...
      <div style="font-family:'Space Grotesk',sans-serif;font-size:1.3rem;font-weight:700;
                  color:#7c3aed;margin-bottom:10px;">Hi! I'm StudyMate</div>
...
<div style="display:flex;height:100vh;padding:14px;gap:12px;overflow:hidden;">
  ...
  <div style="width:270px;min-width:250px;flex-shrink:0;background:#fff;
    ...
    <div style="padding:14px 16px 6px;flex-shrink:0;">
      <a href="/new" style="display:block;text-align:center;text-decoration:none;
    ...
    <div style="padding:10px 14px;border-top:1px solid #e9d5ff;flex-shrink:0;">
      <a href="/logout" style="display:block;text-align:center;text-decoration:none;
  ...
  <div style="flex:1;min-width:0;background:#fff;border-radius:22px;
    ...
    <div style="display:flex;gap:10px;align-items:flex-end;padding:14px 18px;
      ...
      <textarea id="inp" placeholder="Ask StudyMate anything… (Enter to send)" rows="1"
      ...
      <button onclick="send()" id="sbtn" style="
        background:linear-gradient(135deg,#8b5cf6,#a78bfa);color:#fff;
        border:none;border-radius:15px;font-weight:700;font-size:.92rem;
        padding:13px 24px;cursor:pointer;white-space:nowrap;height:50px;
...
setInterval(function(){{
  n=(n+1)%4;
  document.getElementById('dots').textContent='.'.repeat(n||1);
}},400);
...
async def root(request: Request):
    ...
    if u: return RedirectResponse("/chat")
...
async def do_login(request: Request, username: str = Form(...), password: str = Form(...)):
    ...
    resp.set_cookie("sm_session", tok, httponly=True, max_age=86400*7)
    return resp
...
async def do_register(request: Request, username: str = Form(...), password: str = Form(...)):
    ...
    if load_user(username):
        return HTMLResponse(auth_page(error="Username already exists. Please log in.", tab="reg"))
...
async def new_chat(request: Request):
    ...
    return HTMLResponse(chat_page(u, [], items, sid))
...
async def chat_home(request: Request):
    ...
    if items:
        ...
        return HTMLResponse(chat_page(u, history, items, sid))
    sid = str(int(time.time()))
    return HTMLResponse(chat_page(u, [], items, sid))
...
async def api_chat(request: Request):
    ...
    data = load_user(u)
    title = (data or {}).get("chats", {}).get(sid, {}).get("title", msg[:50])
    if len(history) == 2:
        title = msg[:50]
